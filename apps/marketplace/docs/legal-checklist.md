...
- Uploads must not include personal names, faces, case records, support plans, assessment records, disability details, workplace names, or service-user identifiers unless anonymized and reviewed.
...
- MVP should use a marketplace payment service provider. Do not hold seller balances manually.
- Avoid wallet points, platform deposits, or stored-value credits in MVP.
- Record seller payout status, platform fee, payment service fee, refund status, and invoice/receipt metadata.
- Confirm invoice-system handling with an accountant before launch.
